        ...
        {preview && (
          value ? (
            ...
            <button
              ...
              onClick={handleClick}
              disabled={uploading}
              className={`flex-shrink-0 flex flex-col items-center justify-center gap-2 border-2 border-dashed border-gray-200 rounded-lg bg-gray-50 hover:bg-gray-100 hover:border-blue-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-gray-400 hover:text-blue-500 ${previewClassName}`}
              title="Klik untuk upload gambar"
            >
              {uploading ? (
                <svg className="animate-spin w-7 h-7 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              ...
              <span className="text-xs font-medium">{uploading ? `${progress}%` : 'Upload'}</span>
            </button>
          )
        )}

        {/* Upload Button (shown when no preview mode, or alongside preview) */}
        {(!preview || value) && (
